...
#endif
	if (type == R_REQ) {
                int cmp1 = ordo_cmp_clock(ts, _oldest_wts);
                int cmp2 = ordo_cmp_clock(ts, _latest_wts);
		if (cmp1 == ORDO_LESS_THAN || cmp1 == ORDO_UNCERTAIN)
			// the version was already recycled... This should be very rare
			rc = Abort;
		else if (cmp2 == ORDO_GREATER_THAN) {
                        int cmp3 = ordo_cmp_clock(ts, _prewrite_ts);
                        if (cmp3 == ORDO_UNCERTAIN)
                                rc = Abort;
                        else if (_exists_prewrite && cmp3 == ORDO_GREATER_THAN)
			{
				// exists a pending prewrite request before the current read. should wait.
				rc = WAIT;
				buffer_req(R_REQ, txn, false);
				...
				rc = RCOK;
				txn->cur_row = _latest_row;
				if (ordo_cmp_clock(ts,_max_served_rts) == ORDO_GREATER_THAN)
					_max_served_rts = ts;
			}
                }
                else if(cmp2 == ORDO_UNCERTAIN){
                        rc = Abort;
		} else {
			rc = RCOK;
			// ts is between _oldest_wts and _latest_wts, should find the correct version
