...
def parseresults(log_file, plot_data, t, duration, ops_file, abort_file):
    ...
    for line in fp:
        ...
        abrts = (w[3])
        if t == 'swisstm':
            plot_data[t]['abrt_ratio'].append(float(abrts)/(float(w[2])+float(abrts)))
        else:
            plot_data[t]['abrt_ratio'].append(abrts)
        ops_file.write(str(thd) + "    " + str(float(tot_ops)/1000) + "\n")
        abort_file.write(str(thd) + "    " + str(plot_data[t]['abrt_ratio'][-1]) + "\n")
        #print thd
        print tot_ops
...
for test in data:
    ...
    for ur in data[test][0]["update_rate"]:
        ...
        for t in data[test][0]["alg_type"]:
            ...
            execute(data[test][0]["runs_per_test"], data[test][0]["duration"], \
                    t, ur, data[test][0]["initial_size"], data[test][0]["range_size"], out_file, data[test][0]["zipf_dist_val"], data[test][0]["threads"])

            parseresults(out_file, plot_data, t, data[test][0]["duration"], ops_file, abort_file)
