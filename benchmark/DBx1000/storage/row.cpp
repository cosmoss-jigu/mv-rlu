...
#elif CC_ALG == RLU || CC_ALG == MVRLU
    manager = (Row_rlu *) _mm_malloc(sizeof(Row_rlu), 64);
#endif
...
RC row_t::get_row(access_t type, txn_man * txn, row_t *& row) {
	...
	return rc;
#elif CC_ALG == TIMESTAMP || CC_ALG == MVCC || CC_ALG == HEKATON || CC_ALG == RLU || CC_ALG == MVRLU
	uint64_t thd_id = txn->get_thd_id();
...
#elif CC_ALG == TIMESTAMP || CC_ALG == MVCC || CC_ALG == RLU || CC_ALG == MVRLU
